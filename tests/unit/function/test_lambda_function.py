    ...
    "dummy-repo": {
        "codebuild_cfg": {
            "environment_variables": [
                {"name": "foo", "type": "PLAINTEXT", "value": "bar"}
            ]
        }
    }
}
...
@pytest.mark.parametrize(
    "event,expected_status_code,expect_start_build",
    [
        pytest.param(
            {
                "requestPayload": {
        ...
        pytest.param(
            {"requestPayload": {"headers": {"X-GitHub-Event": "starred"}, "body": {}}},
            500,
            False,
            id="invalid_event",
        ),
...
def test_lambda_handler(
    mock_client, mock_repo_cfg, event, expected_status_code, expect_start_build
):
